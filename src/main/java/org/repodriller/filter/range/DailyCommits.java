...
public class DailyCommits implements CommitRange {
	...
	public List<ChangeSet> get(SCM scm) {
		...
		for(ChangeSet cs : all) {
			System.out.println("Commit " + cs.getId() + ": year " + cs.getTime().get(Calendar.YEAR));
	...
	private boolean isFarFromTheLastOne(ChangeSet cs, LinkedList<ChangeSet> filtered) {
		...
		/* More recent commits have larger TimeInMills.
		 * Thus this assumes commits are being handled in recent-to-oldest order.
		 */
		return (daysInMillis <= lastInMillis - currentInMillis);
	}
